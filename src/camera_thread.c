...
static void generate_plate(char *buf) {
    ...
    buf[0] = letters[sys_rand32_get() % 26];
    buf[1] = letters[sys_rand32_get() % 26];
    buf[2] = letters[sys_rand32_get() % 26];
    buf[3] = numbers[sys_rand32_get() % 10];
    buf[4] = letters[sys_rand32_get() % 26];
    buf[5] = numbers[sys_rand32_get() % 10];
    buf[6] = numbers[sys_rand32_get() % 10];
    buf[7] = '\0';
}
    ...
    while (1) {
        ...
        if (zbus_sub_wait(&camera_sub, &chan, K_FOREVER) == 0) {
            if (chan == &camera_trigger_chan) {
                ...
                zbus_chan_pub(&camera_result_chan, &result, K_NO_WAIT);
            }
        }
    }
}
